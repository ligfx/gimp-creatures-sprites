...
check_version ()
{
    MAJOR_HAVE=`echo $1 | awk '{split($0,a,"."); print a[1]}'`
    MAJOR_REQ=`echo $2 | awk '{split($0,a,"."); print a[1]}'`
    MED_HAVE=`echo $1 | awk '{split($0,a,"."); print a[2]}'`
    MED_REQ=`echo $2 | awk '{split($0,a,"."); print a[2]}'`
    MIN_HAVE=`echo $1 | awk '{split($0,a,"."); print a[3]}'`
    MIN_REQ=`echo $2 | awk '{split($0,a,"."); print a[3]}'`
    if expr $MAJOR_HAVE \>= $MAJOR_REQ > /dev/null; then
    	if expr $MED_HAVE \>= $MED_REQ > /dev/null; then
    	    if expr "x$MIN_REQ" = "x" > /dev/null; then
    	    	echo "yes (version $1)"
    	    	return
    	    elif expr $MIN_HAVE \>= $MIN_REQ > /dev/null; then
		echo "yes (version $1)"
        	return
    	    fi
    	fi
